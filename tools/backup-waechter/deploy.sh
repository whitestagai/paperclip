#!/usr/bin/env bash
# Deploy des Backup-Waechters nach ~/.paperclip/scripts/backup-waechter/.
...
SRC="$REPO_ROOT/tools/backup-waechter"
DEST="$HOME/.paperclip/scripts/backup-waechter"
PLIST="de.whitestag.backup-waechter.plist"
...
if ! cmp -s "$SRC/$PLIST" "$ZIEL_PLIST" 2>/dev/null; then
  ...
  echo "  Neu laden mit: launchctl bootout gui/\$UID/de.whitestag.backup-waechter;"
