...
export interface HeartbeatServiceOptions {
  /**
   * Optional plugin broadcast hook. When provided, the heartbeat service
   * invokes `broadcastBeforeAdapterExecute` immediately before calling
   * `adapter.execute` so installed plugins can inject env vars, override
   * runtime config, or block the run. Omit to skip the hook entirely —
   * useful for tests or callers that don't need plugin integration.
   */
