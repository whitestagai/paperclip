#!/usr/bin/env bash
# Deploy des Vault-Spiegels nach ~/.paperclip/scripts/vault-nas-sync/.
...
SRC="$REPO_ROOT/tools/vault-nas-sync"
DEST="$HOME/.paperclip/scripts/vault-nas-sync"
PLIST="de.whitestag.vault-nas-sync.plist"
...
if ! cmp -s "$SRC/$PLIST" "$ZIEL_PLIST" 2>/dev/null; then
  ...
  echo "  Neu laden mit: launchctl bootout gui/\$UID/de.whitestag.vault-nas-sync;"
